...
- **Living v4 (perceptibility) is on branch `living-v4-perceptibility`, NOT yet merged to main** — awaiting Guillermo's review before pushing (push = Cloudflare deploy). It is the implementation of the "WHITE NOON maestro plan" (see `CONCEPT.md` — the three-layer law + Fase-1 audit). Harness 22/22 green, budgets met.
  - **Shipped fixes (all verified with `scripts/audit-probe.mjs` numbers + before/after crops):** Sky day-tint now **warm & visible through the afternoon** (heliostat ANCHORS warmth floor up at 16/18/19.5; tint mask 82%→92%; dead daypart `--fc-tint` CSS rules deleted) — his #1 complaint. Motes are **gold (saturated `--sun`) and ~16**, kept `multiply` (NOTE: `screen`/additive is invisible on the near-white field — do not "fix" it that way). Ocean ripple **crests as a ring** (keyframe mid-stop) and energy now varies width/duration/a visible wake, **not brightness** (the old inversion is gone). Spine cursor bend **contained** (MAXPULL 14→8, gain 0.18→0.12, squared Y-weight, FALLOFF→340) — kills the "weird flick". Moss = deeper visited **border** (32%→40%) + a softened **bloom** tick (14px, curved). Kintsugi gold seam **separated** from the red crack (top 3px→5px, 1.5px). Idle-sigh now drives a **dedicated `--fc-dot-sigh`/`--fc-spine-sigh` slow-breath** (the dot swells ~16% / spine breathes deeper as the field rests) instead of the imperceptible +6% haze — no new fog. Added a 1200×630 **og/social image** (`scripts/make-og.mjs` → `public/og-image.png`) + og/twitter head meta.
  - **Deferred (noted, his call):** card **grain/tooth** (flat cards — art-direction, needs a stacking-context-safe layer); slimming the shipped-but-unused `public/motifs/*` SVGs; populating empty `public/references/`. og:image is a **root-relative** path — if a custom domain is set, consider making it absolute.
